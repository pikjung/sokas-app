...
const BottomNavigationItem: React.FC<BottomNavigationProps> = ({ name, icon: Icon, url, pathname }) => {
  return (
    <a href={url ? url : '#'} className={`w-full focus:text-indigo-500 ${pathname === url ? 'text-indigo-700' : 'text-slate-900'}  hover:text-indigo-500 justify-center inline-block text-center pt-2 pb-1`}>
