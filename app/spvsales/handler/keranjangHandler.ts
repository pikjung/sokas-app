...
export const getKeranjang = async (setKeranjang: any) => {
  try {
    const response = await axios.get(`${apiUrl}/spvsales/cart`, {
      headers: {
...
export const updateKeranjang = async (cartId: string, qty: number) => {
  try {
    const response = await axios.put(`${apiUrl}/spvsales/cart`, { cartId: cartId, qty: qty }, {
      headers: {
        Authorization: `Bearer ${getToken()}`
      }
    })
...
export const updateDiscount = async (cartId: string, discount: string) => {
  try {
    const response = await axios.put(`${apiUrl}/spvsales/cart/discount`, { cartId: cartId, discount: discount }, {
      headers: {
        Authorization: `Bearer ${getToken()}`
      }
    })
...
export const deleteKeranjang = async (cartId: string) => {
  try {
    const response = await axios.delete(`${apiUrl}/spvsales/cart/${cartId}`, {
      headers: {
        Authorization: `Bearer ${getToken()}`
      }
    })
...
export const checkOutKeranjang = async (groupedKeranjang: any) => {
  try {
    const response = await axios.post(`${apiUrl}/spvsales/cart`, groupedKeranjang, {
      headers: {
        Authorization: `Bearer ${getToken()}`
      }
    })
